...
class Homes extends Admin_controller {
  ...
  public function show ($id) {
    ...
    return redirect_message (Cfg::setting ('deploy', 'view', ENVIRONMENT) . 'home/' . $this->obj->id . '-' . rawurlencode (preg_replace ('/[\/%]/u', ' ', $this->obj->title)) . '.html', array ('_flash_info' => ''));
  }
  public function index ($offset = 0) {
    ...
    $limit = 10;
    $total = Home::count (array ('conditions' => $conditions));
    $objs = Home::find ('all', array ('offset' => $offset < $total ? $offset : 0, 'limit' => $limit, 'order' => 'id DESC', 'conditions' => $conditions));
  ...
  public function create () {
    ...
    if (!Home::transaction (function () use (&$obj, $posts, $cover) { return verifyCreateOrm ($obj = Home::create (array_intersect_key ($posts, Home::table ()->columns))) && $obj->cover->put ($cover); }))
  ...
  public function update () {
    ...
    if (!Home::transaction (function () use ($obj, $posts, $cover) { if (!$obj->save () || ($cover && !$obj->cover->put ($cover))) return false; return true; }))
  ...
  private function _validation_create (&$posts, &$cover) {
    if (!isset ($posts['content'])) return '沒有填寫 首頁文章內容！';
    if (!isset ($cover)) return '沒有選擇 首頁文章封面！';
    
    if (!(is_string ($posts['content']) && ($posts['content'] = trim ($posts['content'])))) return '首頁文章內容 格式錯誤！';
    if (!is_upload_image_format ($cover, 20 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png'))) return '首頁文章封面 格式錯誤！';
  ...
  private function _validation_update (&$posts, &$cover, $obj) {
    if (!isset ($posts['content'])) return '沒有填寫 首頁文章內容！';
    if (!((string)$obj->cover || isset ($cover))) return '沒有選擇 首頁文章封面！';

    if (!(is_string ($posts['content']) && ($posts['content'] = trim ($posts['content'])))) return '首頁文章內容 格式錯誤！';
    if ($cover && !is_upload_image_format ($cover, 20 * 1024 * 1024, array ('gif', 'jpeg', 'jpg', 'png'))) return '首頁文章封面 格式錯誤！';
