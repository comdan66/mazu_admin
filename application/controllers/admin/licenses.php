...
class Licenses extends Admin_controller {
  ...
  public function __construct () {
    ...
    if (!User::current ()->in_roles (array ('licensee')))
    ...
    if (in_array ($this->uri->rsegments (2, 0), array ('show', 'edit', 'update', 'destroy', 'is_enabled')))
      if (!(($id = $this->uri->rsegments (3, 0)) && ($this->obj = License::find ('one', array ('conditions' => array ('id = ?', $id))))))
  ...
  public function show ($id) {
    ...
    return redirect_message (Cfg::setting ('deploy', 'view', ENVIRONMENT) . 'licensee/' . $this->obj->id . '-' . rawurlencode (preg_replace ('/[\/%]/u', ' ', $this->obj->title)) . '.html', array ('_flash_info' => ''));
  }
  public function index ($offset = 0) {
    ...
    $limit = 10;
    $total = License::count (array ('conditions' => $conditions));
    $objs = License::find ('all', array ('offset' => $offset < $total ? $offset : 0, 'limit' => $limit, 'order' => 'id DESC', 'conditions' => $conditions));
  ...
  public function create () {
    ...
    if (!License::transaction (function () use (&$obj, $posts, $cover) { return verifyCreateOrm ($obj = License::create (array_intersect_key ($posts, License::table ()->columns))) && $obj->cover->put ($cover); }))
  ...
  public function update () {
    ...
    if (!License::transaction (function () use ($obj, $posts, $cover) { if (!$obj->save () || ($cover && !$obj->cover->put ($cover))) return false; return true; }))
